...
type KingpinCmdClauses struct {
	...
	PlanRoot      *kingpin.CmdClause
	PlanRun       *kingpin.CmdClause
	PlanResume    *kingpin.CmdClause
...
func New(version string, assetRoot string) (*kingpin.Application, *KingpinCmdClauses, *common.MorphOptions) {
	...
	// FIXME: Make this behave like everything else
	planRoot := app.Command("plan", "Create, run and resume plans")
	...
	cmdClauses := &KingpinCmdClauses{
		...
		PlanRoot:      planRoot,
		PlanRun:       runPlanCmd(planRoot.Command("run", "Run an existing plan"), options),
		PlanResume:    resumePlanCmd(planRoot.Command("resume", "Resume an existing plan"), options),
